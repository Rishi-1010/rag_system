...
if ($argc < 2) {
    echo "Usage: php rag_auto.php <command> [arguments]\n";
    echo "Commands:\n";
    echo "  index <file_path> - Index a document (document ID will be auto-generated)\n";
...
switch ($command) {
    case 'index':
        if ($argc < 3) {
            echo "Usage: php rag_auto.php index <file_path>\n";
            exit(1);
        }
        ...
        $documentId = pathinfo($filePath, PATHINFO_FILENAME) . '_' . time();
        
        try {
            $ragService->indexDocument($filePath, $documentId);
            echo "Document indexed successfully with ID: $documentId\n";
        } catch (Exception $e) {
    ...
    case 'ask':
        if ($argc < 3) {
            echo "Usage: php rag_auto.php ask <question>\n";
            exit(1);
        }
        $question = $argv[2];
